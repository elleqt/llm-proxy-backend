...
func TestEndToEnd(t *testing.T) {
	...
	p.webJSON(t, http.MethodGet, "/api/admin/users", "", http.StatusOK, &accounts)
	...
	issued := p.issueToken(t, "laptop")
	secret := issued.Secret
	_, list := p.webCall(t, http.MethodGet, "/api/me/tokens", "")
	...
	p.webJSON(t, http.MethodPost, "/api/admin/policy/preview", `{"rules":["`+p.a.policyName+`:*"]}`, http.StatusOK, &preview)
	if len(preview.Covered) != 1 || preview.Covered[0].Provider != p.a.policyName || preview.Covered[0].Model != p.a.alias {
		t.Fatalf("preview = %+v, want only %s's %s", preview, p.a.policyName, p.a.alias)
	}
	var vendorAccounts []api.ProviderAccount
	p.webJSON(t, http.MethodGet, "/api/admin/providers", "", http.StatusOK, &vendorAccounts)

	// Narrowed to vendor A: A is served and listed, B is refused and hidden.
	p.setPolicy(t, p.adminEmail, p.a.policyName+":*")
	if code := p.chat(t, secret, p.a.alias); code != http.StatusOK {
		t.Fatalf("allowed model: status %d, want 200", code)
	}
	if code := p.chat(t, secret, p.b.alias); code != http.StatusForbidden {
		t.Fatalf("model of a vendor the policy does not grant: status %d, want 403", code)
	}
	if m := p.models(t, secret); !m[p.a.alias] || m[p.b.alias] {
		t.Fatalf("/v1/models = %v, want %s listed and %s hidden", m, p.a.alias, p.b.alias)
	}

	// Widening the policy applies to the same token at once.
	p.setPolicy(t, p.adminEmail, p.a.policyName+":*", p.b.policyName+":*")
	if code := p.chat(t, secret, p.b.alias); code != http.StatusOK {
		t.Fatalf("after widening: status %d, want 200 with the same token", code)
	}

	// Revocation from the cabinet is immediate.
	p.webJSON(t, http.MethodDelete, "/api/me/tokens/"+issued.Token.Id.String(), "", http.StatusNoContent, nil)
	if code := p.chat(t, secret, p.a.alias); code != http.StatusUnauthorized {
	...
	eventually(t, "the cabinet shows both served requests", func() bool {
		p.webJSON(t, http.MethodGet, "/api/me/usage", "", http.StatusOK, &usage)
	...
	old := p.sessionCookie(t)
	p.webJSON(t, http.MethodPost, "/api/auth/logout", "", http.StatusNoContent, nil)
	req, err := http.NewRequest(http.MethodGet, p.webURL+"/api/me", nil)
	if err != nil {
		t.Fatal(err)
	}
	req.AddCookie(old)
	...
	out := p.out.String()
	if n := strings.Count(out, p.adminPassword); n != 1 {
