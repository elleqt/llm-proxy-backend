...
func newAdminFixture(t *testing.T, cfg app.AdminUsersConfig) *adminFixture {
	...
	tokenSvc := app.NewTokenService(f.users, f.tokens, f.audit, clock, discardLogger{})
	f.svc = app.NewAdminUsers(f.users, f.passwords, f.idents, f.sessions, f.activity,
		tokenSvc, testHasher(), f.audit, clock, f.catalog, cfg)
	return f
}
...
func newPerson() identity.User {
	return identity.User{ID: uuid.New(), Kind: identity.KindHuman, Email: "person@example.com",
...
func assertNoSecret(t *testing.T, events []app.AuditEvent, secrets ...string) {
	...
	for _, e := range events {
		...
		for _, s := range secrets {
			if s != "" && strings.Contains(string(raw)+e.Target, s) {
				t.Fatalf("audit event %s carries a secret: %s", e.Action, raw)
			}
		}
	}
}
...
func TestAdminUsersRefuseEveryoneButAnActiveAdmin(t *testing.T) {
	...
	ops := map[string]func(*app.AdminUsers, identity.User) error{
		"ListUsers": func(s *app.AdminUsers, a identity.User) error { _, err := s.ListUsers(ctx, a); return err },
		"GetUser":   func(s *app.AdminUsers, a identity.User) error { _, err := s.GetUser(ctx, a, target); return err },
		...
		"ResetPassword":   func(s *app.AdminUsers, a identity.User) error { _, err := s.ResetPassword(ctx, a, target); return err },
		"ListTokens":      func(s *app.AdminUsers, a identity.User) error { _, err := s.ListTokens(ctx, a, target); return err },
		...
		"Activity":    func(s *app.AdminUsers, a identity.User) error { _, err := s.Activity(ctx, a, target, 10); return err },
		"Catalog":     func(s *app.AdminUsers, a identity.User) error { _, err := s.Catalog(a); return err },
...
func TestUpdatePolicyOfAnIdPUserRefusedWhileTheMappingIsConfigured(t *testing.T) {
	f := newAdminFixture(t, app.AdminUsersConfig{OIDCIssuer: inviteIssuer, GroupMappingConfigured: true})
	target := idpUser()
	f.users.EXPECT().ByID(mock.Anything, target.ID).Return(target, nil)

	rules := []string{"alpha:*"}
	_, err := f.svc.UpdateUser(context.Background(), newAdmin(), target.ID, app.UserChanges{Policy: &rules})
...
func TestUpdatePolicyOfAnIdPUserConvertsItWhenNoMappingIsConfigured(t *testing.T) {
	...
	f.users.EXPECT().UpdateAdminState(mock.Anything, target.ID, mock.Anything).RunAndReturn(func(_ context.Context, _ uuid.UUID, ch app.AdminChange) error {
		change = ch
		return nil
	})
	f.viewAfterUpdate(target)
	events := f.recordAudit()

	rules := []string{"beta:model-*"}
	if _, err := f.svc.UpdateUser(context.Background(), newAdmin(), target.ID, app.UserChanges{Policy: &rules}); err != nil {
...
func TestBlockingAUserDeletesTheirSessions(t *testing.T) {
	...
	f.sessions.EXPECT().DeleteByUser(mock.Anything, target.ID).RunAndReturn(func(context.Context, uuid.UUID) error {
	...
	if _, err := f.svc.UpdateUser(context.Background(), newAdmin(), target.ID, app.UserChanges{Status: &blocked}); err != nil {
		t.Fatalf("UpdateUser: %v", err)
	}
...
func TestRenameWritesOnlyTheNameAndKeepsSessions(t *testing.T) {
	...
	name := " Renamed "
	if _, err := f.svc.UpdateUser(context.Background(), newAdmin(), target.ID, app.UserChanges{DisplayName: &name}); err != nil {
		t.Fatalf("UpdateUser: %v", err)
	}
...
func TestRenewInvitationRefusesWhatNobodyCouldRedeem(t *testing.T) {
	...
	for name, c := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			f := newAdminFixture(t, app.AdminUsersConfig{OIDCIssuer: c.issuer})
			id := uuid.New()
			if c.user != nil {
				id = c.user.ID
				f.users.EXPECT().ByID(mock.Anything, id).Return(*c.user, nil)
			}
			if err := f.svc.RenewInvitation(context.Background(), newAdmin(), id); !errors.Is(err, app.ErrNotInvitable) {
...
func TestRenewInvitationInvitesTheAccountsAddressForTheExactIssuer(t *testing.T) {
	...
	f.idents.EXPECT().Invite(mock.Anything, target.ID, want).Return(nil)
	events := f.recordAudit()

	if err := f.svc.RenewInvitation(context.Background(), newAdmin(), target.ID); err != nil {
...
func TestResetPasswordIssuesATemporaryPasswordAndEndsSessions(t *testing.T) {
	...
	f.users.EXPECT().SetMustChangePassword(mock.Anything, target.ID, true).RunAndReturn(func(context.Context, uuid.UUID, bool) error {
		calls = append(calls, "restrict")
		return nil
	})
	f.passwords.EXPECT().Set(mock.Anything, target.ID, mock.Anything, mock.Anything).
	...
	f.sessions.EXPECT().DeleteByUser(mock.Anything, target.ID).Return(nil)
	events := f.recordAudit()

	temp, err := f.svc.ResetPassword(context.Background(), newAdmin(), target.ID)
	if err != nil {
...
func TestRevokeTokenOfAnotherAccountIsNotFound(t *testing.T) {
	...
	f.tokens.EXPECT().ListByUser(mock.Anything, owner).Return([]credentials.Token{own}, nil)

	if err := f.svc.RevokeToken(context.Background(), newAdmin(), owner, foreign.ID); !errors.Is(err, app.ErrNotFound) {
...
func TestActivityBoundsThePageSize(t *testing.T) {
	for asked, want := range map[int]int{0: app.DefaultActivityLimit, 1: 1, 200: 200, 201: app.MaxActivityLimit} {
		...
		target := newPerson()
		f.users.EXPECT().ByID(mock.Anything, target.ID).Return(target, nil)
		f.activity.EXPECT().RecentUsage(mock.Anything, target.ID, want).Return(nil, nil)
		f.activity.EXPECT().RecentAudit(mock.Anything, target.ID, want).Return(nil, nil)

		if _, err := f.svc.Activity(context.Background(), newAdmin(), target.ID, asked); err != nil {
...
func TestPolicyPreviewAgreesWithTheGateOnAModelServedByTwoProviders(t *testing.T) {
	f := newAdminFixture(t, app.AdminUsersConfig{})
	f.catalog.EXPECT().Models().Return(map[string][]string{
		"alpha": {"shared-model", "solo"},
		"beta":  {"shared-model"},
	})
	f.catalog.EXPECT().ProvidersFor("shared-model").Return([]string{"alpha", "beta"})
	f.catalog.EXPECT().ProvidersFor("solo").Return([]string{"alpha"})
	admin := newAdmin()

	one, err := f.svc.PolicyPreview(admin, []string{"alpha:*", "not a rule"})
	if err != nil {
	...
	both, err := f.svc.PolicyPreview(admin, []string{"alpha:*", "beta:*"})
	if err != nil {
		t.Fatalf("PolicyPreview: %v", err)
	}
