...
// Service issues, lists and revokes API keys.
//
...
func New(users app.UserRepo, tokens app.TokenRepo, audit app.AuditSink, clock app.Clock, logger app.Logger) *Service {
	return &Service{users: users, tokens: tokens, audit: audit, clock: clock, logger: logger}
}
...
// app.MaxLiveTokensPerOwner live keys is refused with ErrTokenLimit.
...
func (s *Service) Revoke(ctx context.Context, actor identity.User, tokenID uuid.UUID) error {
...
func (s *Service) List(ctx context.Context, actor identity.User, owner uuid.UUID) ([]credentials.Token, error) {
	if err := manages(actor, owner); err != nil {
		return nil, err
	}
...
func (s *Service) retract(ctx context.Context, tok credentials.Token, by uuid.UUID, now time.Time, cause error) error {
...
func (s *Service) stranded(tok credentials.Token, cause, failure error, what string) error {
