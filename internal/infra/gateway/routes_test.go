...
func (r *running) send(t *testing.T, method, path, key, body string) (int, string) {
	t.Helper()
	req, err := http.NewRequest(method, r.baseURL+path, strings.NewReader(body))
	if err != nil {
...
func TestDeniedRoutesAreNotServed(t *testing.T) {
	...
	for _, p := range probes {
		if code, body := r.send(t, p[0], p[1], wireSecret, `{"model":"x"}`); code != http.StatusNotFound || body != "" {
	...
	if code, _ := r.send(t, http.MethodGet, "/healthz", "", ""); code != http.StatusOK {
...
func TestWebsocketRelayIsRefused(t *testing.T) {
	...
	r := startWith(t, p)
	wsURL := "ws" + strings.TrimPrefix(r.baseURL, "http") + "/v1/ws"
	...
	if err := r.gateway.PushConfig(open); err != nil {
...
func startVendorPair(t *testing.T) *vendorPair {
	...
	v := &vendorPair{allowed: &faketest.Vendor{Payload: payload}, other: &faketest.Vendor{Payload: payload}}
	allowedSrv, otherSrv := faketest.Start(t, v.allowed), faketest.Start(t, v.other)

	seq := strconv.FormatInt(wireSeq.Add(1), 10)
	allowedName, otherName := "allowed"+seq, "other"+seq
	v.allowedAlias, v.otherAlias, v.sharedAlias = "alias-allowed-"+seq, "alias-other-"+seq, "alias-shared-"+seq
	allowedEntry := faketest.Compatibility(allowedName, allowedSrv.URL, vendorKey, "upstream-allowed-"+seq, v.allowedAlias)
	otherEntry := faketest.Compatibility(otherName, otherSrv.URL, vendorKey, "upstream-other-"+seq, v.otherAlias)
	shared := cliproxyconfig.OpenAICompatibilityModel{Name: "upstream-shared-" + seq, Alias: v.sharedAlias}
	...
	v.running = startWith(t, p)
	awaitProviders(t, v.gateway.catalog, v.allowedAlias, []string{allowedName})
	awaitProviders(t, v.gateway.catalog, v.otherAlias, []string{otherName})
	awaitProviders(t, v.gateway.catalog, v.sharedAlias, []string{allowedName, otherName})
	return v
}

// requests reports how many requests each vendor has received.
func (v *vendorPair) requests() (allowed, other int) {
...
func TestPolicyDecidesWhichVendorARequestReaches(t *testing.T) {
	v := startVendorPair(t)
	for _, model := range []string{v.otherAlias, v.sharedAlias} {
		if code, body := v.send(t, http.MethodPost, "/v1/messages", wireSecret, messages(model)); code != http.StatusForbidden {
			t.Fatalf("POST /v1/messages for %s = %d %s, want 403", model, code, body)
		}
	}
	if n, m := v.requests(); n != 0 || m != 0 {
		t.Fatalf("vendors received %d and %d requests from refused clients, want none", n, m)
	}

	if code, body := v.send(t, http.MethodPost, "/v1/messages", wireSecret, messages(v.allowedAlias)); code != http.StatusOK {
		t.Fatalf("POST /v1/messages for %s = %d %s, want 200", v.allowedAlias, code, body)
	}
	if n, m := v.requests(); n != 1 || m != 0 {
...
func TestRepeatedModelKeysAreRefused(t *testing.T) {
	...
	bodies := map[string]string{
		"allowed, then denied":     `{"model":"` + v.allowedAlias + `",` + rest + `,"model":"` + v.otherAlias + `"}`,
		"denied, then allowed":     `{"model":"` + v.otherAlias + `",` + rest + `,"model":"` + v.allowedAlias + `"}`,
		"a case-variant duplicate": `{"model":"` + v.allowedAlias + `",` + rest + `,"Model":"` + v.otherAlias + `"}`,
		"an escaped duplicate":     `{"model":"` + v.allowedAlias + `",` + rest + `,"mod\u0065l":"` + v.otherAlias + `"}`,
	}
	for what, body := range bodies {
		for _, path := range []string{"/v1/messages", "/v1/chat/completions"} {
			if code, out := v.send(t, http.MethodPost, path, wireSecret, body); code != http.StatusBadRequest {
	...
	if code, out := v.send(t, http.MethodPost, "/v1/messages", wireSecret, messages(v.allowedAlias)); code != http.StatusOK {
...
func TestAccessIsClaimedBeforeTheServerServes(t *testing.T) {
	...
	if res, authErr := g.access.Authenticate(context.Background(), req); authErr == nil {
