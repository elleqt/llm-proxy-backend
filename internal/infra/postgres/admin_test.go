...
func TestAdminRepos(t *testing.T) {
	...
	t.Run("ListAndView", func(t *testing.T) {
		...
		addPending(t, ctx, pool, carol.ID, "issuer-a", carol.Email, time.Hour)
		addPending(t, ctx, pool, dave.ID, "issuer-a", dave.Email, -time.Hour)
		// A leftover invitation beside a link has nothing left to claim.
		addPending(t, ctx, pool, gus.ID, "issuer-a", gus.Email, time.Hour)
		...
		for _, v := range all {
			w := want[v.User.ID]
			if !slices.Equal(v.SignIn, w.signIn) {
				t.Errorf("%s: sign-in = %v, want %v", v.User.DisplayName, v.SignIn, w.signIn)
			}
			if (v.InvitationExpiresAt != nil) != w.invited {
				t.Errorf("%s: invitation expiry = %v, want set: %v", v.User.DisplayName, v.InvitationExpiresAt, w.invited)
			}
		}

		v, err := users.View(ctx, dave.ID)
		if err != nil {
			t.Fatalf("View: %v", err)
		}
		if v.User.Email != dave.Email || v.InvitationExpiresAt == nil || !v.InvitationExpiresAt.Before(time.Now()) {
			t.Fatalf("View = %+v, want the lapsed invitation's expiry", v)
		}
	...
	t.Run("UpdateAdminState", func(t *testing.T) {
		u := human(t, "hana@example.com", "Hana")
		...
		for i, ch := range []app.AdminChange{{DisplayName: &name}, {Status: &blocked}} {
			...
			go func() {
				defer wg.Done()
				errs[i] = users.UpdateAdminState(ctx, u.ID, ch)
			}()
		}
		wg.Wait()
		...
		if err := users.UpdateAdminState(ctx, u.ID, app.AdminChange{Role: &admin, Policy: &policy}); err != nil {
	...
	t.Run("CreateAccount", func(t *testing.T) {
		...
		account := func(actorID uuid.UUID) app.NewAccount {
			u := identity.User{ID: uuid.New(), Kind: identity.KindHuman, Email: "Jo@Example.com", DisplayName: "Jo",
			...
			return app.NewAccount{
				...
				Audit:    app.AuditEvent{ActorID: actorID, Action: "user.create", Target: u.ID.String()},
			}
		}
	...
	t.Run("SessionsDeleteByUserExcept", func(t *testing.T) {
		...
		now := time.Now().UTC()
		for _, s := range []app.Session{
			{IDHash: "iris-kept", UserID: target.ID}, {IDHash: "iris-1", UserID: target.ID},
			{IDHash: "iris-2", UserID: target.ID}, {IDHash: "jon-1", UserID: bystander.ID},
		} {
			s.CreatedAt, s.ExpiresAt = now, now.Add(time.Hour)
			if err := repo.Create(ctx, s); err != nil {
				t.Fatalf("Create: %v", err)
			}
		}
	...
	t.Run("Invite", func(t *testing.T) {
		...
		for name, ttl := range map[string]time.Duration{"expired": -time.Hour, "live": time.Hour} {
			...
			addPending(t, ctx, pool, stale.ID, issuer, "Again-"+name+"@example.com", ttl)
		...
		addPending(t, ctx, pool, other.ID, "https://other.example.com", "ivy@example.com", time.Hour)
		...
		addPending(t, ctx, pool, linked.ID, issuer, "lee@example.com", time.Hour)
	...
	t.Run("RecentUsage", func(t *testing.T) {
		...
		addUsage(t, ctx, pool, user.ID, &tok.ID, at.Add(-3*time.Minute), "alpha", "old")
		...
		e := got[0]
		if !e.At.Equal(at.Add(-time.Minute)) || e.UserID != user.ID || e.TokenID != tok.ID || e.Provider != "beta" ||
...
func addUsage(t *testing.T, ctx context.Context, pool *pgxpool.Pool, userID uuid.UUID, tokenID *uuid.UUID, at time.Time, provider, model string) {
	t.Helper()
