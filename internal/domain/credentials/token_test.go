...
func TestGenerateRefusesLabelsTheRuleForbids(t *testing.T) {
	for _, tc := range []struct {
		...
		if tc.ok {
			if err != nil || secret == "" || tok.Label != tc.label {
				t.Errorf("%s: Generate = %q / %v, want the label accepted", tc.name, tok.Label, err)
			}
		...
		if !errors.Is(err, ErrInvalidLabel) {
			t.Errorf("%s: err = %v, want ErrInvalidLabel", tc.name, err)
		}

		if secret != "" {
			t.Errorf("%s: a secret was minted for a refused label", tc.name)
		}
	}
}
