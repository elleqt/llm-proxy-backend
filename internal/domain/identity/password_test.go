...
func TestPasswordRoundTrip(t *testing.T) {
	...
	require.True(t, VerifyPassword(hash, "correct horse battery staple"), "correct password rejected")
	require.False(t, VerifyPassword(hash, "wrong"), "wrong password accepted")
}
...
func TestHashesAreSalted(t *testing.T) {
	...
	require.NotEqual(t, a, b, "identical hashes for the same password: salt is missing")
}
...
func TestVerifyUsesTheParametersInTheHash(t *testing.T) {
	...
	require.NoError(t, err, "read salt")
	...
	require.True(t, VerifyPassword(hash, "legacy secret"), "hash written with other cost parameters rejected")
	require.False(t, VerifyPassword(hash, "not it"), "wrong password accepted against a hash with other cost parameters")
}
...
func TestCurrentHashRecordsTheRecommendedParameters(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(hash, "$argon2id$v=19$m=19456,t=2,p=1$"),
		"hash = %q, want the OWASP m=19456,t=2,p=1 argon2id encoding", hash)
}
...
func TestVerifyRejectsUnusableHashes(t *testing.T) {
	...
	for name, hash := range map[string]string{
		...
		require.False(t, VerifyPassword(hash, "secret"), "%s: unusable hash %q accepted the password", name, hash)
	}
}
